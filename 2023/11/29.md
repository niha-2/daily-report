...
- チェックアウト時のメール送信機能追加
  - gemのsend-gridを使用
  - gemは安定しないので、使用するのをやめる
  - ローカルの場合は、letter_openerを使うようにして、本番環境でsendgridを使う
  - ローカルでwsl2の場合、下記設定が必要
    - <https://note.com/emetselch1984/n/n23cee998ad22>

- dockerコンテナでの環境変数の設定方法
  - .envファイルで```SENDER_ADDRESS=example@gmail.com```を設定
  - docker-compose.ymlファイルで、environmentに```SENDER_ADDRESS=$SENDER_ADDRESS```を設定
